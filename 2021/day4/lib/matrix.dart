...
class Row {
  ...
  String toString() {
    ...
    for (var cell in cells) {
      value =
          "$value ${cell.marked ? "[" : ""}${cell.value}${cell.marked ? "]" : ""}";
    }
    return value;
  }
}
...
class Matrix {
    ...
    for (var row in rows) {
      ...
      if (column == -1) {
        ...
        if (row.allMarked) {
          ...
          winner =
              !rows.map((row) => row.cells[column]).any((cell) => !cell.marked);
        }
      }
    }
  }
  ...
  int score() {
    var score = rows
        .map((row) => row.cells.fold<int>(
            0,
            (previousValue, cell) =>
                previousValue + (cell.marked ? 0 : cell.value)))
